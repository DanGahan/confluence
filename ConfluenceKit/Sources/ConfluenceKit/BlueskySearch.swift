...
extension BlueskyClient {
    ...
    public func search(auth: BlueskyAuth, query: String, cursor: String? = nil, limit: Int = 25) async -> SearchResults {
        if let cursor {
            guard let data = try? await get(auth: auth, method: "app.bsky.feed.searchPosts",
        ...
        async let actors = try? searchActors(auth: auth, query: query, limit: limit)
        async let postsData = try? get(auth: auth, method: "app.bsky.feed.searchPosts",
    ...
    private func searchActors(auth: BlueskyAuth, query: String, limit: Int) async throws -> [SearchActor] {
        let data = try await get(auth: auth, method: "app.bsky.actor.searchActors", query: query, limit: limit, cursor: nil)
    ...
    private func get(auth: BlueskyAuth, method: String, query: String, limit: Int, cursor: String?) async throws -> Data {
