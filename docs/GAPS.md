...
|---|---|---|---|
| [G1](https://github.com/DanGahan/confluence/issues/104) | **No 429 rate-limit backoff.** SPEC's non-functional requirements demand exponential backoff + user messaging on 429; nothing in either client handles it today. A 429 currently surfaces as a generic server error. | All API client files | Before any feature that raises request volume (live/ticker mode #91 polls every 10–15 s — do G1 first) |
| [G2](https://github.com/DanGahan/confluence/issues/105) | **Bluesky auth is app-password only.** ATProto OAuth is the sanctioned path; app passwords bypass 2FA and will eventually be deprecated. | `BlueskyClient.swift`, `BlueskyAccountStore.swift` | When Bluesky announces deprecation, or before public distribution |
| [G3](https://github.com/DanGahan/confluence/issues/106) | **Engagement is one-way.** Repost/like create records; there's no un-repost/un-like (needs `deleteRecord` on Bluesky, `unreblog`/`unfavourite` on Mastodon). UI shows optimistic state per session only. | `BlueskyEngagement.swift`, `MastodonEngagement.swift`, `PostActionStore` | When the UI grows an "undo" affordance |
| [G4](https://github.com/DanGahan/confluence/issues/107) | **Token refresh is wired per-closure, not centralized.** Each fetcher closure in `FeedView.make…()` carries its own catch-refresh-retry dance (`withSession` helpers). Correct, tested, but duplicated ~6×. | `FeedView.swift` 289–490 | Next time a new endpoint set is added — extract a shared `withFreshBlueskyToken` wrapper into ConfluenceKit then |
...
| # | Gap | Where | Repay when |
|---|---|---|---|
| [G11](https://github.com/DanGahan/confluence/issues/108) | **`FeedView.swift` is 674 lines** and holds all the closure-wiring (`makeFetchers` etc.) — borderline business logic living in a view file. | `FeedView.swift` | Next feature that touches the wiring: extract a `FeedWiring`/composition-root type (still app-side, but its own file) |
| [G12](https://github.com/DanGahan/confluence/issues/109) | `FeedWindowConfigurator` polls `asyncAfter(0.05)` until the window exists to force tab grouping | `ConfluenceApp.swift` | If Apple ships SwiftUI tabbing control; until then it's contained |
| [G13](https://github.com/DanGahan/confluence/issues/110) | `EphemeralSecureStore` is `@unchecked Sendable` (NSLock-guarded dictionary) | `Keychain.swift` | Cosmetic; swap for `Mutex` when minimum toolchain allows |
